/* ==========================================================================
   Quartett — Regelprüfung in Node
   ==========================================================================

   Spielt vollständige Partien gegen die ECHTE `game-service.js` der drei
   Quartett-Spiele — nicht gegen einen Nachbau. Firebase wird durch die
   ...
   Aufruf:
     node pruefe-spiel.js                    alle drei Spiele, je 15 Partien
     node pruefe-spiel.js auto-quartett 30   ein Spiel, eigene Partienzahl
     node pruefe-spiel.js auto-quartett 5 3 karten-vermehren   Mutationsprobe
   ...
   hier nicht belegt — dafür ist `spiele/pflege/pruefe-rules.js` zuständig.
   ========================================================================== */
'use strict';
const fs = require('fs');
...
const NAMENSRAUM = {
  'auto-quartett': 'autoQuartett/',
  'fussball-quartett': 'fussballQuartett/',
  'fussball-vereine-quartett': 'fussballVereineQuartett/'
};
const ALLE = Object.keys(NAMENSRAUM);

const argSpiel = process.argv[2];
const SPIELE = argSpiel && NAMENSRAUM[argSpiel] ? [argSpiel] : ALLE;
...
function ladeQuellen(spiel) {
  const ordner = path.join(WURZEL, spiel);
...
async function pruefeSpiel(spiel) {
  ...
  const kartenPfad = NAMENSRAUM[spiel] + 'geheime_karten';
...
(async () => {
  console.log('');
  console.log('  Quartett — Regelprüfung');
