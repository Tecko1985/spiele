/* ==========================================================================
   ...
   141 Werte über 300 Ticks sind rund 42.000 Kurswerte je Partie. Würde das
   ...
   laufen Kurse und Meldungen auf jedem Handy weiter, auch ganz ohne Netz;
   nur Käufe und Rangliste holen auf, wenn das Netz zurückkommt.
...
const markt = (function () {
  ...
  const TICKS = 300;          // je Partie, unabhängig von der Spieldauer
  const JAHRE = 5;            // simulierter Zeitraum
  const TICKS_JE_JAHR = TICKS / JAHRE;   // 60
  const DT = JAHRE / TICKS;   // Jahresbruchteil je Tick
  ...
  /* ----------------------------------------------------------------------
     ...
     80 %, Anleihen um die 6 %. Über fünf Jahre wird daraus eine Spreizung,
     die eine Partie spannend macht, ohne dass ein einziger Treffer alles
     entscheidet — dafür sorgt zusätzlich die 25-Prozent-Grenze je Wert.
     ---------------------------------------------------------------------- */
  ...
  function erzeuge(saat, werte) {
    ...
    const meldungen = nachrichten.plane(rng, werte, TICKS);
    ...
    /* SCHRITT 2: gemeinsame Marktbewegung. Ein einziger Pfad, an dem alle
       ...
       und es gäbe nie einen Tag, an dem "alles rot" ist — genau die Tage
       machen aber die Stimmung im Bus aus. */
    const marktSchritte = new Float64Array(TICKS + 1);
    for (let t = 1; t <= TICKS; t++) marktSchritte[t] = normal(rng);
    ...
    for (let i = 0; i < gruppenListe.length; i++) {
      const reihe = new Float64Array(TICKS + 1);
      for (let t = 1; t <= TICKS; t++) reihe[t] = normal(rng);
      gruppenSchritte.push(reihe);
    }

    /* SCHRITT 4: Meldungswirkung je Wert und Tick einsammeln.
       Die Wirkung wird über `dauer` Ticks verteilt, nicht sofort gebucht —
       das ist der Grund, warum man auf eine Nachricht noch reagieren kann. */
    const schub = {};      // wertId -> Float64Array(TICKS+1)
    const gewinnSchub = {};
    const profile = {};
    for (const w of werte) {
      schub[w.id] = new Float64Array(TICKS + 1);
      gewinnSchub[w.id] = new Float64Array(TICKS + 1);
    ...
    for (const m of meldungen) {
      ...
      const jeTick = gesamt / m.dauer;
      const gewinnJeTick = m.gewinn ? Math.log(1 + m.gewinn) / m.dauer : 0;
      ...
      for (const w of betroffen) {
        ...
        for (let d = 0; d < m.dauer; d++) {
          const t = m.tick + d;
          if (t > TICKS) break;
          schub[w.id][t] += jeTick * daempfung;
          gewinnSchub[w.id][t] += gewinnJeTick * daempfung;
        }
      }
    }
    ...
    for (const w of werte) {
      ...
      const reihe = new Float64Array(TICKS + 1);
      const gReihe = new Float64Array(TICKS + 1);
      reihe[0] = w.kurs;
  ...
  function begrenze(tick) {
    if (!(tick >= 0)) return 0;
    return tick > TICKS ? TICKS : Math.floor(tick);
  }

  function kurs(lauf, id, tick) {
    const reihe = lauf.kurse[id];
    return reihe ? reihe[begrenze(tick)] : 0;
  }

  /* Veränderung gegenüber dem Vortag im Spielsinn: ein Tick entspricht rund
     sechs Handelstagen, deshalb ist der letzte Tick der passende Vergleich. */
  function veraenderung(lauf, id, tick) {
    const t = begrenze(tick);
    if (t === 0) return 0;
    const reihe = lauf.kurse[id];
  ...
  function seitStart(lauf, id, tick) {
    const reihe = lauf.kurse[id];
    if (!reihe || !reihe[0]) return 0;
    return (reihe[begrenze(tick)] / reihe[0] - 1) * 100;
  }
  ...
  function kgv(lauf, wert, tick) {
    const g = lauf.gewinne[wert.id];
    if (!g || !g[0]) return null;
    const gew = g[begrenze(tick)];
    if (!(gew > 0)) return null;
    return kurs(lauf, wert.id, tick) / gew;
  }

  function gewinnJeAktie(lauf, id, tick) {
    const g = lauf.gewinne[id];
    return g ? g[begrenze(tick)] : 0;
  }
  ...
  function divRendite(lauf, wert, tick) {
    if (!wert.dividende) return 0;
    const k = kurs(lauf, wert.id, tick);
    return k > 0 ? (wert.dividende / k) * 100 : 0;
  }

  function marktkapitalisierung(lauf, wert, tick) {
    if (wert.art === 'krypto') {
      if (!wert.umlaufmenge) return null;
      return kurs(lauf, wert.id, tick) * wert.umlaufmenge;
    }
    return null;
  }

  /* Höchst- und Tiefstand bis zum aktuellen Tick — im Spiel das Gegenstück
     zum 52-Wochen-Hoch. */
  function spanne(lauf, id, tick) {
    const reihe = lauf.kurse[id];
  ...
  /* Meldungen bis einschließlich Tick, neueste zuerst. */
  function meldungenBis(lauf, tick) {
    const t = begrenze(tick);
    const raus = [];
    for (let i = lauf.meldungen.length - 1; i >= 0; i--) {
      if (lauf.meldungen[i].tick <= t) raus.push(lauf.meldungen[i]);
    }
    return raus;
  }

  /* Wie viele Dividendenzahlungen bis zu diesem Tick fällig waren.
     Ausgeschüttet wird einmal je simuliertem Jahr. */
  function dividendenZahlungen(tick) {
    return Math.floor(begrenze(tick) / TICKS_JE_JAHR);
  }

  /* Das simulierte Datum — macht den Zeitraffer sichtbar. */
  function spieljahr(tick) {
    return begrenze(tick) / TICKS_JE_JAHR;
  }

  return {
    TICKS: TICKS,
    JAHRE: JAHRE,
    TICKS_JE_JAHR: TICKS_JE_JAHR,
