...
// gemergter Cache gehalten.
let kategorienCache = null;

async function stelleKategorienBereit() {
  if (kategorienCache) return;
  try {
    kategorienCache = await gameService.ladeKategorienZurBearbeitung();
...
function erzeugeKartenElement(karte, { waehlbar }, kategorien) {
  ...
  const rolleHtml = `<span class="karte-rolle">${escapeHtml(karte.rolle)}</span>`;
  wrapper.innerHTML = `
...
function renderLobby(zustand) {
  ...
  document.getElementById("lobby-modus").textContent = `🛡️ Vereine-Quartett · ${groesseLabel}`;
}

function renderSpiel(zustand) {
  const eigeneKarte = zustand.eigeneKarten[0];
  const amZug = zustand.phase === "amZug";
  const kategorien = effektiveKategorien();
...
async function ladeUndZeigeKartenverwaltung() {
  ...
  let karten;
  try {
    karten = await gameService.ladeKartenZurBearbeitung();
  } catch (e) {
    liste.innerHTML = "";
...
async function oeffneKartenBearbeitung(karte) {
  ...
  let kategorien;
  try {
    kategorien = await gameService.ladeKategorienZurBearbeitung();
  } catch (e) {
...
async function ladeUndZeigeKriterienBearbeitung() {
  ...
  Object.keys(kategorien).forEach(schluessel => {
    ...
    resetBtn.addEventListener("click", async () => {
      await gameService.setzeKategorieZurueck(schluessel);
...
document.getElementById("btn-name-bestaetigen").addEventListener("click", async () => {
  ...
  const ergebnis = ausstehenderModus === "erstellen"
    ? await gameService.erstelleRaum(name, ausstehenderDeckgroesse)
...
document.getElementById("btn-kr-speichern").addEventListener("click", async () => {
  ...
  try {
    for (const key of schluessel) {
      ...
      await gameService.speichereKategorieUebersteuerung(key, { icon, label });
    }
  } catch (e) {
...
document.getElementById("btn-kb-speichern").addEventListener("click", async () => {
  ...
  fehlerEl.textContent = "";
  try {
    await gameService.speichereKartenUebersteuerung(karte.id, daten);
  } catch (e) {
    fehlerEl.textContent = "Speichern fehlgeschlagen.";
    return;
  }
...
document.getElementById("btn-kb-zuruecksetzen").addEventListener("click", async () => {
  ...
  try {
    await gameService.setzeKarteZurueck(karte.id);
  } catch (e) {
