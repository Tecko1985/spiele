...
//   fussballQuartett/raeume/$raumCode               -> öffentlicher Zustand, für alle Mitspieler:innen lesbar
//   fussballQuartett/geheime_karten/$raumCode/$uid  -> private Hand, nur vom eigenen Gerät ODER vom Gastgeber lesbar
...
// Eigener "fussballQuartett/"-Namensraum, getrennt von "autoQuartett/" im selben Firebase-Projekt.

const STORAGE_KEY = "spiele_fussballquartett_raumcode";
...
const RAEUME_PFAD = "fussballQuartett/raeume";
const GEHEIME_KARTEN_PFAD = "fussballQuartett/geheime_karten";
const KARTEN_UEBERSTEUERUNGEN_PFAD = "fussballQuartett/kartenUebersteuerungen";
...
const KATEGORIEN_UEBERSTEUERUNGEN_PFAD = "fussballQuartett/kategorienUebersteuerungen";
...
function holeVollstaendigesDeck(ueberschreibungen) {
  const basis = getMockDeck();
...
/* ⚠️ Aufräumen darf nicht daran hängen, dass jemand danach noch einen Knopf
   ...
   keinen zweiten Löschweg. Der Info-Reiter sagt aber zu: "Mit dem Ende der
...
function ruesteAufraeumen(code, raum) {
  ...
  const daten = { [`${RAEUME_PFAD}/${code}`]: null };
  uids.forEach(uid => { daten[`${GEHEIME_KARTEN_PFAD}/${code}/${uid}`] = null; });
...
function betretRaumLokal(code) {
  ...
  ownHandRef = db.ref(`${GEHEIME_KARTEN_PFAD}/${code}/${eigeneUid}/karten`);
...
async function erstelleRaum(spielerName, deckgroesse) {
...
async function starteSpiel() {
  ...
  const ueberschreibungen = (await db.ref(KARTEN_UEBERSTEUERUNGEN_PFAD).once("value")).val() || {};
  const deck = holeVollstaendigesDeck(ueberschreibungen);
  ...
  const updates = {};
  uids.forEach(uid => {
    updates[`${GEHEIME_KARTEN_PFAD}/${code}/${uid}/karten`] = haende[uid];
    updates[`${RAEUME_PFAD}/${code}/spieler/${uid}/kartenAnzahl`] = haende[uid].length;
    updates[`${RAEUME_PFAD}/${code}/spieler/${uid}/istAusgeschieden`] = false;
  });
  updates[`${RAEUME_PFAD}/${code}/amZugSpielerId`] = uids[Math.floor(Math.random() * uids.length)];
...
async function bestaetigeWeiter() {
  ...
  await db.ref(`${RAEUME_PFAD}/${aktuellerRaumCode}/aktuelleRunde/weiterBestaetigtVon/${eigeneUid}`).set(true);
  return { erfolg: true };
}
...
async function verlasseSpiel() {
  ...
  if (raum.hostId === eigeneUid) {
    ...
    await db.ref(`${RAEUME_PFAD}/${code}/phase`).set("abgebrochen");
...
async function neuesSpiel() {
  ...
  if (raum && code && raum.hostId === eigeneUid) {
    const updates = { [`${RAEUME_PFAD}/${code}`]: null };
    Object.keys(raum.spieler || {}).forEach(uid => {
      updates[`${GEHEIME_KARTEN_PFAD}/${code}/${uid}`] = null;
    });
...
// Keine Zugriffsbeschraenkung: es gibt in diesem Projekt keine personenbezogenen
// Fotos/Namen mehr zu schuetzen (anders als der Familien-Code im Ursprungsprojekt).

async function ladeKartenZurBearbeitung() {
  await authBereit;
  const snap = await db.ref(KARTEN_UEBERSTEUERUNGEN_PFAD).once("value");
  const ueberschreibungen = snap.val() || {};
  return holeVollstaendigesDeck(ueberschreibungen);
}

async function speichereKartenUebersteuerung(kartenId, daten) {
  await authBereit;
  await db.ref(`${KARTEN_UEBERSTEUERUNGEN_PFAD}/${kartenId}`).set(daten);
  return { erfolg: true };
}

async function setzeKarteZurueck(kartenId) {
  await authBereit;
  await db.ref(`${KARTEN_UEBERSTEUERUNGEN_PFAD}/${kartenId}`).remove();
  return { erfolg: true };
}

// Liefert die effektiven Kriterien (Label + Icon), gemergt mit gespeicherten
// Umbenennungen. Ohne Ueberschreibung kommen einfach die Basis-Kriterien aus
// mock-data.js zurueck.
async function ladeKategorienZurBearbeitung() {
  await authBereit;
  const basis = getKategorien();
  const snap = await db.ref(KATEGORIEN_UEBERSTEUERUNGEN_PFAD).once("value");
  const ueberschreibungen = snap.val() || {};
  const ergebnis = {};
...
async function speichereKategorieUebersteuerung(schluessel, daten) {
  await authBereit;
  await db.ref(`${KATEGORIEN_UEBERSTEUERUNGEN_PFAD}/${schluessel}`).set(daten);
  return { erfolg: true };
}

async function setzeKategorieZurueck(schluessel) {
  await authBereit;
  await db.ref(`${KATEGORIEN_UEBERSTEUERUNGEN_PFAD}/${schluessel}`).remove();
  return { erfolg: true };
}
...
async function entferneSpielerUndVerteileKartenAlsHost(verlassenerUid) {
  ...
  try {
    ...
    const verlasseneHandSnap = await db.ref(`${GEHEIME_KARTEN_PFAD}/${code}/${verlassenerUid}/karten`).once("value");
    ...
    if (verbleibendeUids.length === 0) {
      updates[`${RAEUME_PFAD}/${code}/phase`] = "abgebrochen";
    } else {
      const schnappschuesse = await Promise.all(
        verbleibendeUids.map(uid => db.ref(`${GEHEIME_KARTEN_PFAD}/${code}/${uid}/karten`).once("value"))
      );
      verbleibendeUids.forEach((uid, i) => {
        ...
        updates[`${GEHEIME_KARTEN_PFAD}/${code}/${uid}/karten`] = neueHand;
        updates[`${RAEUME_PFAD}/${code}/spieler/${uid}/kartenAnzahl`] = neueHand.length;
      });
    ...
    updates[`${RAEUME_PFAD}/${code}/spieler/${verlassenerUid}`] = null;
    updates[`${GEHEIME_KARTEN_PFAD}/${code}/${verlassenerUid}`] = null;
    updates[`${RAEUME_PFAD}/${code}/austrittAnfragen/${verlassenerUid}`] = null;
    ...
    if (raum.austrittAnfrageUid === verlassenerUid || !raum.spieler[raum.austrittAnfrageUid]) {
      updates[`${RAEUME_PFAD}/${code}/austrittAnfrageUid`] = null;
    }
...
async function fuehreVergleichsRundeAlsHost(kategorieSchluessel, teilnehmerUids, kartenIndex, bisherigerPott) {
  const code = aktuellerRaumCode;
  const schnappschuesse = await Promise.all(
    teilnehmerUids.map(uid => db.ref(`${GEHEIME_KARTEN_PFAD}/${code}/${uid}/karten`).once("value"))
  );
  ...
  if (teilnehmerMitKarte.length === 0) {
    ...
    await db.ref(`${RAEUME_PFAD}/${code}/aktuelleRunde`).update({
...
async function fuehreRundenTransferAusAlsHost() {
  ...
  const schnappschuesse = await Promise.all(
    beteiligteUids.map(uid => db.ref(`${GEHEIME_KARTEN_PFAD}/${code}/${uid}/karten`).once("value"))
  );
  ...
  beteiligteUids.forEach((uid, i) => {
    ...
    updates[`${GEHEIME_KARTEN_PFAD}/${code}/${uid}/karten`] = hand;
  });
  const gewinnerRestHand = updates[`${GEHEIME_KARTEN_PFAD}/${code}/${gewinnerUid}/karten`] || [];
  updates[`${GEHEIME_KARTEN_PFAD}/${code}/${gewinnerUid}/karten`] = gewinnerRestHand.concat(gewonneneKarten);

  let nochAktiveAnzahl = 0;
  let letzterAktiverUid = null;
  Object.keys(raum.spieler).forEach(uid => {
    const aktuelleAnzahl = beteiligteUids.includes(uid)
      ? updates[`${GEHEIME_KARTEN_PFAD}/${code}/${uid}/karten`].length
      : raum.spieler[uid].kartenAnzahl;
    const ausgeschieden = aktuelleAnzahl === 0;
    updates[`${RAEUME_PFAD}/${code}/spieler/${uid}/kartenAnzahl`] = aktuelleAnzahl;
    updates[`${RAEUME_PFAD}/${code}/spieler/${uid}/istAusgeschieden`] = ausgeschieden;
    if (!ausgeschieden) {
      nochAktiveAnzahl++;
  ...
  if (nochAktiveAnzahl <= 1) {
    ...
    updates[`${RAEUME_PFAD}/${code}/aktuelleRunde`] = { gewaehlteKategorie: null, ausgespielteKarten: null, gewinnerSpielerId: null };
  }
...
function planeBotZugFallsNoetig(botUid) {
  ...
  setTimeout(async () => {
    ...
    const handSnap = await db.ref(`${GEHEIME_KARTEN_PFAD}/${aktuellerRaumCode}/${botUid}/karten`).once("value");
